...
function stylePanel(el: HTMLElement, grip: HTMLElement, pos: number) {
    const pressed = pos > 0;
    const t = Math.max(-1, Math.min(1, pos / 6));
    el.style.transform = `translateY(${pos}px) scaleY(${1 - Math.abs(t) * 0.012})`;
    ...
    if (pressed) {
        ...
        grip.style.boxShadow = `inset 0 ${2 + t}px ${6 + t * 2}px rgba(0,0,0,0.95)`;
        return;
    }
    ...
    el.style.boxShadow = `0 ${2 + lift * 2}px ${8 + lift * 4}px rgba(0,0,0,0.9), 0 -1px 4px rgba(0,0,0,0.5), inset 0 ${1 + lift}px ${4 + lift * 2}px rgba(255,232,${120 + lift * 20},${0.3 + lift * 0.15}), inset 0 -2px 6px rgba(0,0,0,0.5)`;
...
export default function ArchToggle({ value, onChange }: ArchToggleProps) {
    ...
    useEffect(() => {
        ...
        const paintPanels = () => {
            if (panelTopRef.current && gripTopRef.current) {
                stylePanel(panelTopRef.current, gripTopRef.current, s.top.pos);
            }
            if (panelBotRef.current && gripBotRef.current) {
                stylePanel(panelBotRef.current, gripBotRef.current, s.bot.pos);
            }
        };
        ...
        const tick = (ts: number) => {
            ...
            (["top", "bot"] as const).forEach((key) => {
                const state = s[key];
                const force = -STIFFNESS * (state.pos - state.target) - DAMPING * state.vel;
                state.vel += (force / MASS) * dt;
                state.pos += state.vel * dt;

                if (Math.abs(state.pos - state.target) > 0.005 || Math.abs(state.vel) > 0.005) {
                    moving = true;
                }
            });
    ...
    const handleToggle = (nextValue: "residential" | "commercial") => {
    ...
    return (
        <div className="arch-toggle-container">
            ...
            <div
                className="label label-top"
                style={{ opacity: isRes ? 1 : 0.3 }}
            ...
            <div
                ...
                onClick={() => handleToggle(isRes ? "commercial" : "residential")}
            >
                <div className="panel panel-top" ref={panelTopRef}>
                    <div className="grip" ref={gripTopRef} />
                </div>
                <div className="panel panel-bot" ref={panelBotRef}>
                    <div className="grip" ref={gripBotRef} />
                </div>
            </div>
            <div
                className="label label-bot"
                style={{ opacity: isRes ? 0.3 : 1 }}
